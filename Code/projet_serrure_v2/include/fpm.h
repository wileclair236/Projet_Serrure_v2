...
enum class FPMLedControl:uint8_t
{
    Clignotement_Respiration = 0x01,
    Clignotement_Flash = 0x02,
    Allume = 0x03,
    Eteint = 0x04,
    Allume_Graduel = 0x04,
    Eteint_Graduel = 0x05
};
enum class FPMLedColour:uint8_t
{
    Blanc,
    Rouge,
    Bleu,
...
public:
    ...
    FPMStatus ledConfigure(FPMLedControl controlCode, uint8_t speed, FPMLedColour colour, uint8_t numCycles = 255);
    ...
    void Lecteur_Empreinte_Digital_Setup(int RX_FINGER_PIN, int TX_FINGER_PIN,int IRQ_FINGER_PIN = -1, int BAUD_FINGER = 56700, SerialConfig uart_config = SERIAL_8N1);
    bool getFreeId(int16_t *fid);
    bool Ajout_empreinte_digitale(int16_t fid);
    bool Supprimer_empreinte_digitale(int fid);
    bool Recherche_empreinte_correspondante(uint16_t &fidTrouve);
    bool get_nbr_empreinte(void);
...
extern FPM finger;
extern volatile bool Finger_irqFlag; // Flag d'interruption
#endif
