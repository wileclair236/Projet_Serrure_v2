...
// Déclaration du flag d'interruption
volatile bool Finger_irqFlag = false;

// Fonction d'interruption pour la détection RFID
void IRAM_ATTR Finger_handler()
{
    Finger_irqFlag = true;
    // Serial.println("----========================>>>>>>");
}

FPM::FPM(Stream *ss) : port(ss), password(FPM_DEFAULT_PASSWORD),
...
bool FPM::verifyPassword(uint32_t pwd)
{
    ...
    buffer[1] = (password >> 24) & 0xff;
    buffer[2] = (password >> 16) & 0xff;
    buffer[3] = (password >> 8) & 0xff;
    buffer[4] = password & 0xff;
...
FPMStatus FPM::setPassword(uint32_t pwd)
{
    ...
    buffer[1] = (pwd >> 24) & 0xff;
    buffer[2] = (pwd >> 16) & 0xff;
    buffer[3] = (pwd >> 8) & 0xff;
    buffer[4] = pwd & 0xff;
...
FPMStatus FPM::ledConfigure(FPMLedControl controlCode, uint8_t speed, FPMLedColour colour, uint8_t numCycles)
{
    buffer[0] = FPM_LEDCONTROL;
    buffer[1] = static_cast<uint8_t>(controlCode);
    buffer[2] = speed;
    buffer[3] = static_cast<uint8_t>(colour);
    buffer[4] = numCycles;
...
FPMStatus FPM::setParam(FPMParameter param, uint8_t value)
{
    ...
    buffer[1] = static_cast<uint8_t>(param);
    buffer[2] = value;
...
FPMStatus FPM::readParams(FPMSystemParams *params)
{
    ...
    {
        if (params != NULL)
            memcpy(params, &sysParams, FPM_SYS_PARAMS_LEN);
        return FPMStatus::OK;
    }
...
FPMStatus FPM::readProductInfo(FPMProductInfo *info)
{
    ...
    if (confirmCode != FPMStatus::OK)
        return confirmCode;
    if (readLen != FPM_PRODUCT_INFO_LEN)
        return FPMStatus::READ_ERROR;
...
FPMStatus FPM::writePacket(uint8_t *srcBuffer, Stream *srcStream, uint16_t *writeLen, uint8_t pktId)
{
    ...
    if (srcStream != NULL)
    {
        ...
        while ((uint32_t)(millis() - lastRead) < FPM_DEFAULT_TIMEOUT)
        {
            ...
            if (srcStream->available() < toWrite)
                continue;
...
FPMStatus FPM::readPacket(uint8_t *destBuffer, Stream *destStream, uint16_t *readLen, uint8_t *pktId)
{
    ...
    while ((uint32_t)(millis() - lastRead) < FPM_DEFAULT_TIMEOUT)
    {
        ...
        case FPMState::READ_CHECKSUM:
        {
            ...
            if (readLen != NULL)
                *readLen = packetLen - FPM_CHECKSUM_LENGTH;
...
void FPM::Lecteur_Empreinte_Digital_Setup(int RX_FINGER_PIN, int TX_FINGER_PIN, int IRQ_FINGER_PIN, int BAUD_FINGER, SerialConfig uart_config)
{
    Serial.println("Lecteur d'empreinte digital setup");
    fserial.begin(BAUD_FINGER, SERIAL_8N1, RX_FINGER_PIN, TX_FINGER_PIN);
    delay(2000);
    begin();
    pinMode(IRQ_FINGER_PIN, INPUT_PULLUP);                                           // Mode d'entrée pour l'IRQ
    attachInterrupt(digitalPinToInterrupt(IRQ_FINGER_PIN), Finger_handler, FALLING); // Attacher l'interruption
    get_nbr_empreinte();
}

bool FPM::getFreeId(int16_t *fid)
{
    for (int page = 0; page < (sysParams.capacity / FPM_TEMPLATES_PER_PAGE) + 1; page++)
    {
        FPMStatus status = getFreeIndex(page, fid);
        ...
        default:
            ...
            Serial.printf("getFreeIndex(%d): error 0x%X\n\r", page, static_cast<uint16_t>(status));
...
bool FPM::Ajout_empreinte_digitale(int16_t fid)
{
    ...
    for (int i = 0; i < NUM_SNAPSHOTS; i++)
    {
        ...
        while (status != FPMStatus::OK)
        {
            ...
            default:
                ...
                Serial.printf("getImage(): error 0x%X", static_cast<uint16_t>(status));
                break;
            }
        ...
        default:
            ...
            Serial.printf("image2Tz(%d): error 0x%X\n\r", i + 1, static_cast<uint16_t>(status));
            return false;
        }
    ...
    default:
        ...
        Serial.printf("createModel(): error 0x%X\n\r", static_cast<uint16_t>(status));

        return false;
    }

    status = storeTemplate(fid);
    switch (status)
    {
    case FPMStatus::OK:
        Serial.printf("Template stored at ID %d!\n\r", fid);
        break;

    case FPMStatus::BADLOCATION:

        Serial.printf("Could not store in that location %d!\n\r", fid);
        return false;

    default:

        Serial.printf("storeModel(): error 0x%X", static_cast<uint16_t>(status));
...
bool FPM::Supprimer_empreinte_digitale(int fid)
{
    FPMStatus status = deleteTemplate(fid);
    ...
    default:
        Serial.printf("deleteTemplate(): error 0x%X\n\r", static_cast<uint16_t>(status));
        return false;
    }
...
bool FPM::Recherche_empreinte_correspondante(uint16_t &fidTrouve)
{
    FPMStatus status;

    /* Take a snapshot of the input finger */
    Serial.println("Place a finger.");
    finger.ledConfigure(FPMLedControl::Allume_Graduel, 50, FPMLedColour::Violet);
    finger.ledOn();
    status = getImage();
    ...
    case FPMStatus::OK:
        Serial.println("Image taken");
        finger.ledConfigure(FPMLedControl::Allume, 40, FPMLedColour::Vert);
        break;

    case FPMStatus::NOFINGER:
        finger.ledConfigure(FPMLedControl::Clignotement_Flash, 40, FPMLedColour::Jaune);
        Serial.println(".");
        break;

    default:
        /* allow retries even when an error happens */
        Serial.printf("getImage(): error 0x%X\n\r", static_cast<uint16_t>(status));
        break;
    }

    finger.ledConfigure(FPMLedControl::Clignotement_Respiration, 50, FPMLedColour::Jaune);
    finger.ledOn();
    for (int i = 0; i < 30; i++)
    {
    ...
    uint16_t fid, score;
    status = searchDatabase(&fid, &score);

    switch (status)
    {
    case FPMStatus::OK:
        Serial.printf("Found a match at ID #%u with confidence %u\n\r", fid, score);
        fidTrouve = fid;
        return true;
        break;
    ...
    case FPMStatus::NOTFOUND:
        ...
        finger.ledConfigure(FPMLedControl::Clignotement_Respiration, 50, FPMLedColour::Rouge, 30);
        finger.ledOn();
    ...
    default:
        ...
        Serial.printf("searchDatabase(): error 0x%X\n\r", static_cast<uint16_t>(status));
        return false;
    }
...
bool FPM::get_nbr_empreinte(void)
{
    uint16_t count;

    FPMStatus status = getTemplateCount(&count);

    switch (status)
    {
    case FPMStatus::OK:

        Serial.printf("%u prints in sensor database\n\r", count);
        break;

    default:
        Serial.printf("getTemplateCount(): error 0x%X\n\r", static_cast<uint16_t>(status));
        return false;
    }
